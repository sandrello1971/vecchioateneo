...
<div style="max-width:760px;">
    <a href="{{ route('admin.courses.sessions.index', $course) }}" style="font-size:0.8rem; color:#55B1AE; text-decoration:none;">&larr; Sessioni</a>
    ...
    <p style="color:#6B7280; font-size:0.85rem; margin-bottom:18px;">
        {{ $session->scheduled_at?->format('d/m/Y H:i') }} &middot;
        {{ $session->duration_minutes }} min &middot;
    ...
    @else
    <form action="{{ route('admin.courses.sessions.mark', [$course, $session]) }}" method="POST">
        @csrf
        ...
        <table style="width:100%; border-collapse:collapse; font-size:0.88rem;">
            <thead>
                <tr style="text-align:left; color:#6B7280; border-bottom:2px solid #E5E7EB;">
                    <th style="padding:9px 8px; width:60px;">Presente</th>
                    <th style="padding:9px 8px;">Discente</th>
                    <th style="padding:9px 8px; width:150px;">Ore (opz.)</th>
                </tr>
            </thead>
            <tbody>
                @php $default = round(($session->duration_minutes ?? 0)/60, 2); @endphp
                @foreach($students as $student)
                    @php $rec = $marked->get($student->id); @endphp
                    <tr style="border-bottom:1px solid #F0F0F0;">
                        <td style="padding:9px 8px; text-align:center;">
                            <input type="checkbox" class="pres-chk" name="present[]" value="{{ $student->id }}" @checked($rec) style="width:18px; height:18px;">
                        </td>
                        <td style="padding:9px 8px; color:#1A1F1F;">{{ $student->name }} <span style="color:#9CA3AF; font-size:0.78rem;">{{ $student->email }}</span></td>
                        <td style="padding:9px 8px;">
                            <input type="number" step="0.25" min="0" name="hours[{{ $student->id }}]" value="{{ $rec?->hours_credited ?? $default }}" style="width:90px; padding:5px 8px; border:1px solid #D1D5DB; border-radius:5px; font-size:0.85rem;">
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        <p style="color:#9CA3AF; font-size:0.78rem; margin:10px 0 16px;">Le ore vuote usano la durata della sessione ({{ $default }}h). Salvando, gli assenti (deselezionati) vengono rimossi dal registro di questa sessione.</p>
