...
<div style="max-width:760px;">
    <div style="margin-bottom:8px;"><a href="{{ route('student.classi.messaggi.index', $class) }}" style="color:#8A9696; text-decoration:none; font-size:0.85rem;">&larr; Messaggi</a></div>
    <h1 style="font-size:1.25rem; font-weight:700; color:#1A1F1F;">{{ $conversation->teacher->name ?? '—' }}</h1>
    ...
    <form method="POST" action="{{ route('student.classi.messaggi.reply', [$class, $conversation]) }}" style="display:flex; gap:8px;">
        @csrf
        ...
        <button style="padding:0 18px; background:#55B1AE; color:white; border:none; border-radius:10px; font-weight:600; cursor:pointer;">Invia</button>
    </form>
</div>
@endsection
